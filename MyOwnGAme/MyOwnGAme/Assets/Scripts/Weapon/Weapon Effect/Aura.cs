        ...
        foreach (KeyValuePair<EnemyStats, float> item in affectedTargetsCopy)
		{
            ...
            if (item.Value <= 0)
            {
                ...
                else
                {
					...
					affectedTargets[item.Key] = stats.cooldown*weapon.Owner.Stats.cooldown;
                    item.Key.TakeDamage(GetDamage(), transform.position,stats.knockback);
                    weapon.ApplyBuffs(item.Key);

                    if (stats.hitEffect)
                    {
                        Destroy(Instantiate(stats.hitEffect, item.Key.transform.position, Quaternion.identity), 5f);
                    }
                }
            }
        }
